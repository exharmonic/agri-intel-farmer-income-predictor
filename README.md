...
├── agriintel.ipynb               # Full pipeline: EDA → Feature Engineering → Modelling → Bonus Tasks
├── requirements.txt
└── .gitignore
```
...
|---|---|
| Ridge Regression (baseline) | ~30.26% |
| LightGBM (default params) | ~21.54% |
| **LightGBM + Optuna tuning** | **~21.10%** |

The tuned LightGBM cuts error by ~9 percentage points vs baseline. Train/val MAPE spread across folds stays under 2pp — no meaningful overfitting.
...
The competition data alone wasn't enough. 14 district-level features were pulled from public sources — APMC mandi prices, satellite NDVI, soil NPK, climate stress indicators, cold storage access, bank branch density — all keyed on `DISTRICT` and merged into both train and test. Features 2–17 in the final top 30 are almost entirely from this enrichment. The raw district-level data ended up driving the model more than the engineered transforms of the original columns.
...
- **Tuning:** Optuna Bayesian search over 40 trials. Parameters tuned: `num_leaves`, `learning_rate`, `min_child_samples`, `subsample`, `colsample_bytree`, `reg_alpha`, `reg_lambda`.
...
| 2 | `Total_Land_For_Agriculture` | Original data |
| 3 | `Mandi_Arrival_Volume_Tonnes` | External enrichment |
| 4 | `Soil_Phosphorus_P_kg_ha` | External enrichment |
| 5 | `Soil_pH_Level` | External enrichment |
| 6 | `District_Crop_Diversification_Index` | External enrichment |
| 7 | `Peak_NDVI_Kharif` | External enrichment |
